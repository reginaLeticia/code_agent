...
class RolePlaying:
    ...
    def __init__(
        ...
        codeagent_prompt_template = "CodeAgent is a code review company powered by multiple intelligent agents, such as chief executive officer, chief product officer, code reviewer, security analyst, etc, with a multi-agent organizational structure and the mission of \"building a autonomous codeagent for company to do code review for commits\"."
        print("I find you", task_prompt)
        
        sys_msg_meta_dicts = [dict(codeagent_prompt=codeagent_prompt_template, task=task_prompt)] * 2
    ...
    def init_chat(self, phase_type: PhaseType = None,
        ...
        # refactored ChatDev
        print("stop", self.assistant_agent.role_name,placeholders)
        print(phase_prompt.format(**({"assistant_role": self.assistant_agent.role_name} | placeholders)))
        if self.assistant_agent.role_name=="Code Reviewer":
            import ipdb
            # ipdb.set_trace()
